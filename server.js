...
app.post('/api/interpret', async (req, res) => {
    const { primaryHexagram, secondaryHexagram, changingLines, question } = req.body;

    if (!primaryHexagram) {
        return res.status(400).json({ error: 'Primary hexagram is required.' });
    }
    ...
    const userQuestionText = question && question.trim() ? `Вопрошающий размышляет вот о чем: "${question}"\n` : '';
    ...
    try {
        ...
        res.status(500).json({ error: "Не удалось получить толкование от ИИ." });
    }
});
